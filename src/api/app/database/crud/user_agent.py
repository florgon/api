...
def get_by_string(db: AsyncSession, user_agent_string: str) -> UserAgent:
...
def get_by_id(db: AsyncSession, user_agent_id: int) -> UserAgent | None:
    return db.query(UserAgent).filter(UserAgent.id == user_agent_id).first()


def get_or_create_by_string(db: AsyncSession, user_agent_string: str) -> UserAgent:
